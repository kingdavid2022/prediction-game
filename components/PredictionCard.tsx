      ...
      <div className="w-[100%] h-[50%] flex flex-col items-start justify-center mt-[3%]">
        ...
        <div className=" w-[100%] flex items-center justify-between">
          <span className="text-black text-[0.8rem]">
            Participate for {time.getHours()>12?(time.getHours() - 12).toString():(time.getHours()<10?`0${time.getHours()}` : time.getHours().toString())}:{time.getMinutes()<10? `0${time.getMinutes()}`:time.getMinutes().toString()}{time.getHours()>=12?'PM':'AM'} {time.toLocaleDateString("en-US")}
          </span>
          <button
            onClick={onclick}
